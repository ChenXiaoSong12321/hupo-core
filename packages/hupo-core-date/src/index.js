...
// 开始于
export const startOf = (date, units, isStartOf = true) => {
...
// 结束于
export const endOf = (...arg) => startOf(...arg, false)

// 是否相同
export const isSame = (date1, date2, units) => {
...
export const isAfter = (date1, date2, units) => U.parseDate(date2) < startOf(date1, units)

// date2 is before to date1
export const isBefore = (date1, date2, units) => endOf(date1, units) < U.parseDate(date2)

// 增加时间
export const add = (date, number, units) => {
  const parseDate = U.parseDate(date)
  const unit = U.prettyUnit(units)
  const $y = parseDate.getFullYear()
  const $M = parseDate.getMonth()
...
// 减少时间
export const subtract = (date, number, units) => add(date, number * -1, units)

export const diff = (date1, date2, units) => {
